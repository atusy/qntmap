...
The results are saved as a binary file and the png format images in `clustering` directory below the directory containing mapping data.
...
cls_imgs[1:2] %>>%
  ...
  map(function(.x) {
    expand.grid(
      ...
      mutate(
        fill = rgb(r = c(.x[, , 1]), g = c(.x[, , 2]), b = c(.x[, , 3]))
      )
  }) %>>%
