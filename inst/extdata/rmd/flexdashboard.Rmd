...
if (!exists("params")) {
  params <- list(xmap = NULL, qnt = NULL, deadtime = 0)
}
...
xmap <- read_xmap(dir_xmap, DT = params$deadtime)
...
xmap_df <- reactive({
  if (input$xmap_action != "Summarize") return(.env$xmap$df)
  if (!is.null(input$xmap_brush))
    return(summarize_box(xmap, input$xmap_brush, .env, "xmap"))
  if (!is.null(input$xmap_click))
    return(summarize_click(xmap, input$xmap_click, .env, "xmap"))
  .env$xmap$df
})

xmap_df_latest <- reactive({
  xmap_df() %>>%
    head(3) %>>%
...
cluster_out <- reactive({
  ...
  isolate(if (input$cluster_run != 0L) {
    ...
    structure(
      transmute(xmap, x, y, cluster = "Not yet"), 
      class = c("qm_cluster", "data.frame")
    )
  })
})
```
