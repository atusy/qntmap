...
xmap <- function() xmap0
# xmap <- reactive(read_xmap(dir_xmap, DT = params$deadtime))
qnt <- read_qnt(dir_qnt, saving = FALSE)
epma <- reactive(tidy_epma(qnt, xmap()) %>>% qntmap:::prioritize("id"))
.margin <- c(-.5, .5)
...
```{r xmap-menu}
select_element_and_range("xmap", .env$xmap$elint)
select_color("xmap")
...
select_element_and_range("qmap", .env$qmap$elem)
select_color("qmap")
