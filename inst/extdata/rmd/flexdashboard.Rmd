...
.env$cluster$summary <- .env$cluster$result %>>%
  ...
  mutate(ID = 0L, Area = "Whole") %>>%
  select("ID", "Area", "x", "y", everything())
.env$quantify$element <- qnt$elm %>>%
  filter(elint %in% names(xmap())) %>>%
  `[[`("elem")
.env$quantify$result <- xmap()[c("x", "y")] %>>%
  mutate(!!!(setNames(as.list(numeric(length(.env$quantify$element))), .env$quantify$element))) %>>%
  structure(class = c("qntmap", class(.)))
.env$quantify$summary <- .env$quantify$result %>>%
  summarize_if(is.numeric, mean) %>>%
  mutate(ID = 0L, Area = "Whole") %>>%
  select("ID", "Area", "x", "y", everything())
...
observe_action("quantify", input, ranges, range_x, range_y)
```
...
xmap_squished <- reactive(squish(xmap()[[input$xmap_elem]], xmap_zlim()))
...
```{r quantify-menu}
actionButton(
  "quantify_run", label = "Quantify Maps", width = "90%",
  style = "background: #EE0000;"
)
```
...
# splitLayout(
#   selectInput(
#     "quantify_elem", label = "Element", 
#     choices = .env$quantify$element,
#     selectize = FALSE
#   ),
#   numericInput("quantify_min", "Min", value = NA_real_),
#   numericInput("quantify_max", "Max", value = NA_real_),
#   cellWidths = 100
# )

select_element_and_range("quantify", .env$quantify$element)

select_color("quantify")

select_action("quantify_action")
```
...
quantify_df <- reactive(summarize_action(quantify_out, input, "quantify", .env))

quantify_df_latest <- reactive(summarize_latest(quantify_df))

renderTable(quantify_df_latest(), align = "r")
```


```{r quantify-run}
quantify_out <- reactive({
  input$quantify_run
  isolate(if (input$quantify_run != 0L && input$cluster_run != 0L) {
    .env$quantify$result <- quantify(
...
```{r quantify-zlim}
quantify_zlim <- reactive({
  zrange <- range(quantify_out()[[input$quantify_elem]])
  c(
    `if`(is.na(input$quantify_min) || input$quantify_min < zrange[1L], zrange[1L], input$quantify_min),
    `if`(is.na(input$quantify_max) || input$quantify_max > zrange[2L], zrange[2L], input$quantify_max)
  )
})

quantify_squished <- reactive(squish(quantify_out()[[input$quantify_elem]], quantify_zlim()))

quantify_img <- reactive(qntmap:::as_img(
  qntmap:::lookup[[input$quantify_color]](quantify_squished(), from = quantify_zlim()),
  range_y[2L], range_x[2L]
))
```

```{r quantify-heatmap}
output$quantify_heatmap <- map_render(quantify_img, ranges, .margin, quantify_zlim, input, "quantify")
...
renderPlot(
  qntmap:::gghist.numeric(
    unlist(quantify_out()[[input$quantify_elem]], use.names = FALSE),
    .min = input$quantify_min, .max = input$quantify_max,
    colors = input$quantify_color
  )
)
```
