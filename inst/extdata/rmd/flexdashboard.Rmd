...
.chart-wrapper .chart-stage {overflow: visible;}
...
deadtime <- params$deadtime * is.character(params$xmap)
xmap_dir <- `if`(is.null(params$xmap), file.path(example, ".map/1"), params$xmap)
qnt_dir <- `if`(is.null(params$qnt), file.path(example, ".qnt"), params$qnt)
```
...
.env <- new.env()
xmap0 <- read_xmap(xmap_dir, DT = deadtime)
xmap <- function() xmap0
# xmap <- reactive(read_xmap(xmap_dir, DT = deadtime))
qnt <- read_qnt(qnt_dir, saving = FALSE)
epma <- tidy_epma(qnt, xmap()) %>>% qntmap:::prioritize("id")
.margin <- c(-.5, .5)
range_x <- range(xmap()$x)
range_y <- range(xmap()$y)

.env$xmap$elint <- setdiff(names(xmap()), c("x", "y"))
.env$xmap$summary <- xmap() %>>%
  summarize_if(is.numeric, mean) %>>%
  mutate(ID = 0L, Area = "Whole") %>>%
  select("ID", "Area", "x", "y", everything())
.env$qmap[c("elem", "elint")] <- qnt$elm %>>%
  filter(elint %in% !!.env$xmap$elint) %>>%
  select(elem, elint) %>>%
  as.list %>>%
  map(qntmap:::prioritize, qntmap:::.component)
...
## xmap-menu {.tabset data-width=200}

### Visual

```{r xmap-menu}
menu_map("xmap", .env$xmap$elint, dropdown = drop_map("xmap"))
...
xmap_squished <- squish_react("xmap", xmap, xmap_zlim, input)
...
```{r qnt-phase-list-actions, eval = FALSE}
splitLayout(
  actionButton(
    "qnt_phase_list_down", label = "Download",
    style = "background: #EE0000;"
  ),
  actionButton(
    "qnt_phase_list_up", label = "Upload", style = "background: #EE0000;"
  )
)
```


```{r qnt-phase-list-view, cache=FALSE}
...
## check-menu {data-width=200, .tabset}
...
```{r select-outliers}
picker_input(
  inputId = "outlier_elint",
  label = "Element to plot", 
  choices = .env$qmap$elint,
  selected = .env$qmap$elint[[1L]]
)
...
```{r find-fine-grained, cache = FALSE}
...
outlier_plot <- reactive({
  epma %>>%
    filter(elint == !!input$outlier_elint) %>>%
    mutate(
      good = !(phase %in% !!input$outlier),
      weight = as.integer(good),
      alpha = weight * .7 + .3
    ) %>>%
    filter((!!input$outlier_action != "Filter") | good) %>>%
    ggplot() + 
    ggtitle(input$outlier_elint) +
    outlier_gg_layers
})

renderPlot(outlier_plot())
```
...
```{r tidy-epma, eval = TRUE, cache=FALSE}
renderDT(epma)
```
